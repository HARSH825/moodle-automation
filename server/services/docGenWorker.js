...
import {
  BorderStyle, Document, Packer, Paragraph, TextRun, HeadingLevel,
    ...
    if (relatedFiles.length > 0) {
      ...
      if (downloadedPath && file.fileName.endsWith('.docx')) {
        try {
          ...
          console.log("extraction err :"+extractErr);
        }
      }
    }
...
function horizontalLine() {
  return new Paragraph({
    border: {
      bottom: { color: 'BFBFBF', style: BorderStyle.SINGLE, size: 4 },
    },
    spacing: { after: 200 },
  });
}
...
function styledHeading(text) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text, bold: true, color: "2E74B5", size: 28 }),
    ],
  });
}
    ...
    sections: [{
      ...
      children: [
        new Paragraph({
          ...
          children: [new TextRun({ text: "Experiment", bold: true, size: 30, color: "2E74B5" })],
        }),
        new Paragraph({ text: "" }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 200 },
          children: [new TextRun({ text: sections.title || assignmentDetails.title, bold: true, size: 34 })],
        }),
        horizontalLine(),
        new Table({
          width: { size: 100, type: WidthType.PERCENTAGE },
          borders: { top: { style: BorderStyle.SINGLE, size: 8, color: "4472C4" }, bottom: { style: BorderStyle.SINGLE, size: 8, color: "4472C4" }, left: { style: BorderStyle.SINGLE, size: 8, color: "4472C4" }, right: { style: BorderStyle.SINGLE, size: 8, color: "4472C4" }, insideHorizontal: { style: BorderStyle.SINGLE, size: 4, color: "4472C4" }, insideVertical: { style: BorderStyle.SINGLE, size: 4, color: "4472C4" } },
          rows: [
            ...
            new TableRow({
              ...
              children: [
                new TableCell({
                  children: [
                    new Paragraph({
                      children: [new TextRun({ text: label, bold: true, size: 24, color: "2E74B5" })],
...
async function processDocumentGeneration(job) {
  ...
  try {
    ...
    if (!cookies) throw new Error('No valid session found. Please login first.');
    ...
    for (let i = 0; i < totalAssignments; i += BATCH_SIZE) {
      const batch = selectedAssignments.slice(i, i + BATCH_SIZE);
      const batchResults = await Promise.all(batch.map(async (assignment) => {
        ...
        if (error) throw new Error(`Failed to upload ${assignmentDetails.title}: ${error.message}`);
        const { data: urlData } = supabase.storage.from('gendocs').getPublicUrl(`assignments/${fileName}`);
        return {
      ...
      job.progress(Math.round(((Math.min(i + BATCH_SIZE, totalAssignments)) / totalAssignments) * 100));
    }
...
documentGenQueue.process('generate-documents', 6, processDocumentGeneration);
