...
//cache invalidtn 
export const deleteSession = async (username) => {
  try {
    await delCache(CACHE_KEYS.SESSION(username));
    return true;
  } catch (error) {
    console.error(`Failed to delete session for ${username}:`, error);
    return false;
  }
};

export const deleteCourses = async (username) => {
  try {
    await delCache(CACHE_KEYS.COURSES(username));
    return true;
  } catch (error) {
    console.error(`Failed to delete courses for ${username}:`, error);
    return false;
  }
};
